...
happyReduction_9 (HappyTerminal (TokenProp happy_var_1))
	 =  HappyAbsSyn7
		 (Var happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	TokenOr -> cont 11;
	TokenAnd -> cont 12;
	TokenProp happy_dollar_dollar -> cont 13;
	TokenLP -> cont 14;
	TokenRP -> cont 15;
	_ -> happyError' (tk:tks)
	}
...
data Token = TokenD | TokenB | TokenNot | TokenOr | TokenAnd | TokenProp Prop | TokenLP | TokenRP deriving Show
...
lexer :: String -> [Token]
lexer [] = []
lexer ('<':'>':cs) = TokenD : lexer cs
lexer ('[':']':cs) = TokenB : lexer cs
...
lexer (c:cs)
	| isSpace c = lexer cs
	| isIden c = let (p,ps) = span isIden (c:cs) in TokenProp p : lexer ps

isIden x = isAlpha x || isDigit x || (x == '_')
